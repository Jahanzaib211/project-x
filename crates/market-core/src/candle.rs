...
pub fn candles(
    ...
    let oldest_index = current_index.saturating_sub((count as u64).saturating_sub(1));
    ...
    for index in oldest_index..=current_index {
        ...
        let last_tick = open_tick
            ...
            .saturating_sub(1)
            .min(latest_tick);
        out.push(candle_over(instrument, open_tick, last_tick));
    }
    Ok(out)
}

/// The single candle covering `[open_tick, last_tick]`.
fn candle_over(instrument: &Instrument, open_tick: u64, last_tick: u64) -> Candle {
    let open = mid_raw(instrument, open_tick);
    let mut high = open;
    let mut low = open;
    let mut close = open;
...
mod tests {
    ...
    /// rather than underflowing into candles from before the epoch.
    #[test]
    fn a_window_at_the_start_of_time_clamps_instead_of_underflowing() {
        let built = candles(eurusd(), one_minute(), 10, 100).unwrap();
